...
//
// Host in-memory NVS + minimal esp/FreeRTOS shims for the rule_store host
// test. Single-threaded; the FreeRTOS mutex/task calls are no-ops (the flush
// "task" never runs — rule_store_flush_now() flushes synchronously).
#include "nvs.h"
#include "nvs_flash.h"
#include "esp_timer.h"
...
// ── In-memory store (one namespace; the test uses only rule_store's) ────────
...
BaseType_t xTaskCreate(void (*)(void*), const char*, uint32_t, void*, UBaseType_t, TaskHandle_t*) {
    return pdPASS;   // the flush task never runs; flush_now() is synchronous
}
...
extern "C" int  nvs_stub_rule_count(void) {
    int n = 0;
    for (auto& kv : g_blobs) if (kv.first.size() >= 2 && kv.first[0] == 'r' && kv.first[1] == '_') n++;
    return n;
}
extern "C" void nvs_stub_set_schema(uint16_t v) { g_u16["schema_ver"] = v; }
// Corrupt the first stored rule blob (flip a payload byte → CRC mismatch).
extern "C" bool nvs_stub_corrupt_first_rule(void) {
    for (auto& kv : g_blobs)
        if (kv.first.size() >= 2 && kv.first[0] == 'r' && kv.first[1] == '_' && !kv.second.empty()) {
            kv.second[0] ^= 0xFF;
            return true;
        }
    return false;
